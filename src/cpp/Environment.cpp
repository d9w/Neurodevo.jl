...
Environment::Environment(vector<int> inp_lengths, GRN soma_grn, GRN axon_grn) {
    ...
    for (int y=0; y < lengths[1]; y++) {
      for (int z=0; z < lengths[2]; z++) {
        if ((z == 4 && (x == 0 || x == lengths[0]-1 || y == 0 || y == lengths[1]-1))
          ...
          id = somas.size();
          soma_map[{x, y, z}] = id;
          somas.push_back(Soma(soma_grn, axon_grn, {x, y, z}, id));
        }
      }
    }
  }
...
void Environment::set_random_connectivity(int seed) {
  ...
  for (auto& soma : somas) {
    if (soma.position[2] != lengths[2]-1) {
      for (int i=1; i<AXON_MAX_NUMBER; i++) {
  ...
  for (auto& soma : somas) {
    if (soma.position[2] != lengths[2]-1) {
      soma.axons[0].position = {rand() % X_SIZE, rand() % Y_SIZE, Z_SIZE-1};
      soma.axons[1].position = {rand() % X_SIZE, rand() % Y_SIZE, Z_SIZE-1};
      for (int a=2; a<soma.axons.size(); a++) {
        int x = rand() % X_SIZE;
        int y = rand() % Y_SIZE;
        if (!(x == 0 || x == X_SIZE-1)) {
          y = rand()>RAND_MAX/2.0 ? 0 : Y_SIZE-1;
        }
...
void Environment::develop_grns(double reward) {
  ...
  for (auto& soma : somas) {
    soma_signal = soma.grn.proteins[GRN_OUTPUT_COMM].concentration;
    ...
    for (auto& axon : soma.axons) {
      ...
      axon.evolve(morphogens[soma.position[0]][soma.position[1]][soma.position[2]],
                  soma_concentration, soma_signal);
    }
  }
}
    ...
    if (soma.fire()) {
      for (auto& axon : soma.axons) {
        ...
        if (rec_soma != NULL && rec_soma != &soma && rec_soma->position[2] != 0) {
          ...
          double nt_amount = soma.grn.proteins[GRN_OUTPUT_NT].concentration;
          double nt_thresh = soma.grn.proteins[GRN_OUTPUT_NT_T].concentration;
